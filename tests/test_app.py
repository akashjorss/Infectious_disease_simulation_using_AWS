...
@pytest.mark.skip(reason="Setup mock for SSM")
def test_lambda(mocker: MockFixture):
    ...
    ssm_response = {'Parameters': [{'Name': '/simulation_lambda/bootstrap_script_path', 'Type': 'String',
                                    'Value': 's3://path/bootstrap.sh'},
                                   {'Name': '/simulation_lambda/instance_type', 'Type': 'String', 'Value': 'c4.xlarge'},
                                   {'Name': '/simulation_lambda/log_path', 'Type': 'String',
                                    'Value': 's3://path'},
                                   {'Name': '/simulation_lambda/simulation_app', 'Type': 'String',
                                    'Value': 's3://path/wordcount.py'}]}
    mock_boto = mocker.patch("boto3.client")
    mock_boto.get_parameters_by_path.return_value = MagicMock(ssm_response)
